...
@Component
public class GetRegNoFromColors implements ICommand {

	private ParkingLotSvc parkingSvc;

	@Autowired
	public GetRegNoFromColors(ParkingLotSvc parkingSvc) {
	...
	public String execute(List<String> params) throws ParkingLotException {
		if (params.size() == 1) {
			return ResponseUtil.getStringFromList(parkingSvc.getRegIdsFromColor(params.get(0)));
		} else {
